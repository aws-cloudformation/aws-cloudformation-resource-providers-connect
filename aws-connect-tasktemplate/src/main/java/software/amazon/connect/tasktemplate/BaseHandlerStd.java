...
public abstract class BaseHandlerStd extends BaseHandler<CallbackContext> {
    ...
    protected static void handleCommonExceptions(final Exception ex, final Logger logger) {
        if (ex instanceof ResourceNotFoundException) {
            ...
            for(PropertyValidationExceptionProperty property : ((PropertyValidationException) ex).propertyList()){
                if(StringUtils.equals(property.reason().name(), PropertyValidationExceptionReason.RESOURCE_NAME_ALREADY_EXISTS.name())){
                    throw new CfnAlreadyExistsException( DuplicateResourceException.builder().message(property.message()).build());
                }
                else{
                    messages.add(property.message());
                }
            }
            throw new CfnInvalidRequestException(messages.toString());
